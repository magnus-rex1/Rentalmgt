...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $departs = $_POST["departs"];
        ...
        // Prepare and bind
        $stmt = $conn->prepare("INSERT INTO tenants (tenant_name, rental, departs) VALUES (?, ?, ?)");
        if ($stmt === false) {
            die("Failed to prepare statement: " . $conn->error);
        }

        $null = NULL; // Used for handling BLOB data
        $stmt->bind_param("sss", $tenant_name, $rental, $departs);

        // Execute the query
        if ($stmt->execute()) {
            $success_message = "New tenant added successfully";
        } else {
                ...
                <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
                    <div class="w-full">
                        ...
                        <?php if ($success_message != ""): ?>
                            <div id="message" class="fade-out text-purple-600">
...
<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
    ...
    <label class="block mt-4 text-sm">
        <span class="text-gray-700 dark:text-gray-400">Rental Occupied</span>
        <input
            class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
            placeholder=""
            name="rental"
    ...
    >
        Upload
                        ...
                        <p class="mt-4">
                            <a class="text-sm font-medium text-purple-600 dark:text-purple-400 hover:underline" href="./add_rentals.php">
                                Wanna Manage Rentals? Do it Here 
                            </a>
                            or
